...
const DeleteUserForm = ({
  ...
  const handleDestroyerUser = (e: FormEvent) => {
    e.preventDefault()
    ...
    UserService.destroyUser(state.user_id)
      .then((res) => {
        if(res.status === 200) {
            ...
            console.error('Unexpected status error while destroying user: ', res.status);
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if(user) {
        setState((prevState) => ({
            ...
            full_name: handleUserFullName(user.first_name, user.middle_name, user.last_name, user.suffix_name)
        }));
    } else {
        setState((prevState) => ({
            ...prevState,
  ...
  return (
    <>
    <form ref={formRef} onSubmit={handleDestroyerUser}>
      <div className="row">
